...
anonymity = {4: "elite", 2: "anonymous", 1: "transparent"}
    ...
    def collect(self):
        if self.source_name == "proxydb":
            if self.anonymity == "transparent":
                self.anonymity = int(
                    [i for i, j in anonymity.items() if j == "transparent"][0])
            elif self.anonymity == "anonymous":
                self.anonymity = int(
                    [i for i, j in anonymity.items() if j == "anonymous"][0])
            else:
                self.anonymity = int(
                    [i for i, j in anonymity.items() if j == "elite"][0])

            url = proxy_source[
                "proxydb"] + "&protocol={}&country={}&anonlvl={}".format(
                    self.proxy_type, self.country, self.anonymity)
        ...
        if self.source_name == "geonode":
            proxies_list = []
            url = proxy_source[
                "geonode"] + "limit=200&page=1&sort_by=lastChecked&sort_type=desc&filterLastChecked=60&&country={}&protocols={}&anonymityLevel={}".format(
                    self.country, self.proxy_type, self.anonymity)
            urlcontent = requests.get(url, headers=generate_header())
            tmp_proxies_list = urlcontent.json()
            for i in range(0, len(tmp_proxies_list["data"])):
                proxies_list.append(tmp_proxies_list["data"][i]["ip"] + ":" +
                                    tmp_proxies_list["data"][i]["port"])
            return proxies_list
            ...
            for i in range(1, 3):
                url = proxy_source[
                    "jiangxianli"] + "page=1&orderby&order_by=created_at&order_rule=DESC"
                urlcontent = requests.get(url, headers=generate_header())
                for i in range(0, len(urlcontent.json()["data"]["data"])):
                    proxies_list.append(
                        urlcontent.json()["data"]["data"][i]["ip"] + ":" +
                        urlcontent.json()['data']['data'][i]["port"])
            return proxies_list
            ...
            if self.proxy_type == "https":
                tmp_proxies_list = []
                for i in range(1, 5):
                    url = proxy_source["nimadaili"] + "https/" + "{}/".format(
                        i)
                    urlcontent = requests.get(url, headers=generate_header())
                    proxy_pattern = re.compile("\d+\.\d+\.\d+\.\d+:\d+")
                    tmp_proxies_list.append(
                        proxy_pattern.findall(urlcontent.text))
                proxies_list = [
                    item for sublist in tmp_proxies_list for item in sublist
                ]
                return proxies_list
            ...
            proxies_list = []
            url = proxy_source[
                "proxy-list"] + "type={}&anon={}&country={}".format(
                    self.proxy_type, self.anonymity, self.country)
            urlcontent = requests.get(url, headers=generate_header())
            proxy_pattern = re.compile("\d+\.\d+\.\d+\.\d+:\d+")
...
if __name__ == "__main__":
    ...
    proxies_list.append(
        proxies(source_name="proxydb",
                country="US",
                proxy_type="http",
                anonymity="elite").collect())
    proxies_list.append(
        proxies(source_name="proxydb",
                country="US",
                proxy_type="http",
                anonymity="anonymouse").collect())
    proxies_list.append(
        proxies(source_name="proxydb",
                country="US",
                proxy_type="http",
                anonymity="transparent").collect())
