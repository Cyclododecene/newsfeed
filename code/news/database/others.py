...
class VGEG(object):
    ...
    columns_name_vgeg = ['date', 'showOffset', 'iaShowId', 'station', 'showName', 'iaClipUrl',
    ...
    def __init__(self, query_date:str="2021-01-01", domain:str=None, raw:bool = False, proxy:dict = None):
    ...
    def _query_list(self) -> list:
        ...
        print("[+] Scraping data from GDELT Project...")
        page = pd.read_csv(url,  sep = " ", engine = "c", na_filter = False, low_memory = False, names = ["url"])
    ...
    def _download_file(self, url:str="http://data.gdeltproject.org/gdeltv3/iatv/vgegv2/BBCNEWS_20200601_000000_BBC_World_News.vgeg.v2.json.gz"):
        download_url = url
        time.sleep(0.25)
        try:
            response = requests.get(download_url, headers = self._generate_header(), proxies =self.proxy, 
