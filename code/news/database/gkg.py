...
class Gkg_V1(object):
    ...
    columns_name = ['DATE', 'NUMARTS', 'COUNTS', 'THEMES', 'LOCATIONS', 'PERSONS',
...
class Gkg_V2(object):
    ...
    columns_name = ['GKGRECORDID', 'V2.1DATE', ' V2SOURCECOLLECTIONIDENTIFIER', ' V2SOURCECOMMONNAME', 'V2DOCUMENTIDENTIFIER', ' V1COUNTS', ' V2COUNTS', ' V1THEMES', 
                    ...
                    'V1TONE', 'V2ENHANCEDDATES', 'V2GCAM', ' V2SHARINGIMAGE', ' V2RELATEDIMAGES', ' V2SOCIALIMAGEEMBEDS', ' V2SOCIALVIDEOEMBEDS', ' V2QUOTATIONS',
    ...
    def _query_list(self) -> list:
        ...
        if self.translation == True:
            print("[+] Scraping data from GDELT Project...")
            page = pd.read_csv("http://data.gdeltproject.org/gdeltv2/masterfilelist-translation.txt", sep = " ", 
                                engine = "c", na_filter = False, low_memory = False, names = ["a", "b", "url"])
            ...
            print("[+] Scraping data from GDELT Project...")
            page = pd.read_csv("http://data.gdeltproject.org/gdeltv2/masterfilelist.txt", sep = " ", 
                                engine = "c", na_filter = False, low_memory = False, names = ["a", "b", "url"])
            url_list = list(page[page["url"].str.contains("gkg")]["url"])
            del page
            download_url_list = list(filter(lambda x: x[37:51] >= self.start_date and x[37:51] < self.end_date, url_list))
    ...
    def _download_file(self, url:str="http://data.gdeltproject.org/gdeltv2/20220108000000.gkg.csv.zip"):
        download_url = url
        time.sleep(0.001)
        try:
