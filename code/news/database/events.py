...
class Event_V2(object):
    ...
    header_events = ['GLOBALEVENTID', 'SQLDATE', 'MonthYear', 'Year', 'FractionDate', 'Actor1Code', 'Actor1Name', 
                    'Actor1CountryCode', 'Actor1KnownGroupCode', 'Actor1EthnicCode', 'Actor1Religion1Code', 'Actor1Religion2Code', 
                    ...
                    'NumArticles', 'AvgTone', 'Actor1Geo_Type', 'Actor1Geo_FullName', 'Actor1Geo_CountryCode', 'Actor1Geo_ADM1Code', 'Actor1Geo_ADM2Code', 
                    'Actor1Geo_Lat', 'Actor1Geo_Long', 'Actor1Geo_FeatureID', 'Actor2Geo_Type', 'Actor2Geo_FullName', 'Actor2Geo_CountryCode', 
                    'Actor2Geo_ADM1Code', 'Actor2Geo_ADM2Code', 'Actor2Geo_Lat', 'Actor2Geo_Long', 'Actor2Geo_FeatureID', 'ActionGeo_Type', 
                    'ActionGeo_FullName', 'ActionGeo_CountryCode', 'ActionGeo_ADM1Code', 'ActionGeo_ADM2Code', 'ActionGeo_Lat', 'ActionGeo_Long', 
                    'ActionGeo_FeatureID', 'DATEADDED', 'SOURCEURL']

    header_mentions = ['GLOBALEVENTID', 'EventTimeDate', 'MentionTimeDate', 'MentionType', 'MentionSourceName', 'Mentiondentifier', 'SentenceID',
                        'Actor1CharOffset', 'Actor2CharOffset', 'ActionCharOffset', 'InRawText', 'Confidence', 'MentionDocLen', 'MentionDocTone', 
                        'MentionDocTranslationInfo', "Extras"]


    def __init__(self, start_date:str = "2020-01-01", end_date:str = "2021-12-31", table:str = "events", proxy:dict = None):
    ...
    def _query_list(self) -> list:
        ...
        if self.table != "events" and self.table != "mentions":
            return ValueError("Wrong table name, Event_V2 is only used for querying Event and Mentions")
        else:
            print("[+] Getting data from GDELT Project...")
    ...
    def query(self):
        ...
        try:
            ...
            results.reset_index(drop = True, inplace = True)
            if self.table == "events":
                results.columns = self.header_events
                return results
            else:
                results.columns = self.header_mentions
                return results
        except Exception as e:
            return e
